...
Разрезает index.html на отдельные блоки T123 для Тильды — по секциям сайта.
    ...
    python3 tilda/split-blocks.py index.html --outdir tilda/blocks \
        --scope vf --img-base https://valerabkrv.github.io/gasup/img/ \
...
SECTIONS = [
    ...
    ('CALC',     'calc',     'Калькулятор экономии на газе'),
    ('PRICE',    'price',    'Прайс на установку ГБО'),
    ('OPTIONS',  'options',  'Дополнительные опции'),
    ('SERVICE',  'service',  'ТО, диагностика и ремонт'),
    ('GIBDD',    'gibdd',    'Оформление в ГИБДД'),
    ('STEPS',    'steps',    'Как проходит установка'),
    ('WHY',      'why',      'Почему GASUP'),
    ('FAQ',      'faq',      'Вопросы и ответы'),
    ('CONTACTS', 'contacts', 'Контакты и заявка'),
    ('FOOTER',   'footer',   'Подвал, кнопка вверх, липкая панель'),
    ('MODAL',    'modal',    'Своя форма-модалка (не нужна, если ставим поп-ап Тильды)'),
]


def wrap(scope, name, title, inner):
    return ('<!-- %s | блок сайта GASUP. Порядок блоков менять можно, '
...
def main():
    ...
    styles = ('<!-- Стили и шрифты сайта GASUP. Этот блок должен быть ПЕРВЫМ на странице.\n'
